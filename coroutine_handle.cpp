...
struct task
{
    struct promise_type
    {
        promise_type() { std::cout << "1.create promie object\n"; }
        ...
        void return_void() { std::cout << "12.coroutine don't return value, so return_void is called\n"; }
...
struct awaiter
{
    ...
    void await_resume() { std::cout << "11.5 coroutine resumed, continue execcute coroutine body now, the thread id=" << std::this_thread::get_id() << "\n"; };
};
...
// 5.begin to execute coroutine body, the thread id=127349872932800
...
// 11.coroutine resumed, continue execcute coroutine body now, the thread id=127349800568512
